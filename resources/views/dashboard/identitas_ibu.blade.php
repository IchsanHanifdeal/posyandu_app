          ...
          @foreach (['Daftar_ibu_hamil'] as $item)
            <div class="flex flex-col w-full border-back rounded-xl">
              ...
              <div class="flex flex-col gap-3 p-5 pt-0 divide-y rounded-b-xl sm:p-7">
                              ...
                              <div class="modal-box">
                                ...
                                <div class="flex flex-col w-full gap-3 !h-full mt-3 rounded-lg overflow-hidden">
                                  @if ($item->user->foto_profil === null)
                                    <img id="foto_profil_preview_{{ $item->id_user }}" src="https://ui-avatars.com/api/?name={{ urlencode($item->user->nama) }}"
                                      class="border size-full" alt="{{ $item->user->nama }}">
                                  @else
                                    <img id="foto_profil_preview_{{ $item->id_user }}" src="{{ asset('storage/foto_profil/' . $item->user->foto_profil) }}"
                                      class="border size-full" alt="{{ $item->user->nama }}">
                                  @endif
                              ...
                              <div class="max-w-6xl p-8 transition-all duration-300 ease-in-out transform scale-95 bg-white shadow-xl modal-box rounded-xl hover:scale-100">
                                ...
                                <div class="flex justify-center mb-6">
                                  <div class="w-40 h-40 overflow-hidden bg-pink-100 border-4 border-pink-300 rounded-full shadow-lg">
                                    @if ($item->user->foto_profil === null)
                                      <img id="foto_profil_preview_{{ $item->id_user }}" src="https://ui-avatars.com/api/?name={{ urlencode($item->user->nama) }}"
                                        class="object-cover w-full h-full" alt="{{ $item->user->nama }}">
                                    @else
                                      <img id="foto_profil_preview_{{ $item->id_user }}" src="{{ asset('storage/foto_profil/' . $item->user->foto_profil) }}"
                                        class="object-cover w-full h-full" alt="{{ $item->user->nama }}">
                                    @endif
                ...
                <td class="flex items-center gap-4">
                  <!-- Trigger Button -->
                  <x-lucide-book-user class="cursor-pointer size-5 hover:stroke-green-500" onclick="document.getElementById('detail_modal_{{ $item->id_user }}').showModal();" />
                  ...
                  <dialog id="detail_modal_{{ $item->id_user }}" class="modal modal-bottom sm:modal-middle">
                    <div class="max-w-6xl p-8 transition-all duration-300 ease-in-out transform scale-95 bg-white shadow-xl modal-box rounded-xl hover:scale-100">
                      ...
                      <div class="flex justify-center mb-6">
                        @php
                          $imagePath = $item->foto_profil ? asset('storage/foto_profil/' . $item->foto_profil) : 'https://ui-avatars.com/api/?name=' . urlencode($item->user->nama);
                        @endphp
                        <div class="w-40 h-40 overflow-hidden bg-pink-100 border-4 border-pink-300 rounded-full shadow-lg">
